...
market_data <- read_delim("data/market return.txt", 
                           "\t", escape_double = FALSE, col_types = cols(Trdmnt = col_date(format = "%Y/%m/%d")), 
                           trim_ws = TRUE)
...
for (i in 1:nmonth){
  ...
  x <- beta_factor$beta [trading_data$Trdmnt == trading_lastmonth]
...
write.csv(seaport,file ="results\\liangjia\\beta-portfolio.csv",)
write.csv(results_des,file ="results\\liangjia\\beta-result.csv",)
