...
#trading_data <- read_csv("d:/Users/XuranZENG/Desktop/data/trading data.csv")
#View(trading_data)
...
pricing_factor_data <- read_delim("sample/pricing factor data.txt", 
...
financial_stock_code <- read_csv("sample/financial stock code.txt", 
...
i <- 1
ratio <- 0.2
trading_months <- as.Date("2005-01-01")+months(seq(0,180,3))  ##2020-01-01
nseason <- length(trading_months)
##nseason是投资的月数,trading_months放的是交易时间
...
for (i in 1:nseason){
  x <- pricing_factor_data$F100301B  [pricing_factor_data$Accper == trading_months[i]]
  ...
  seashort$vol <- trading_data$Msmvttl[is.element(trading_data$Stkcd,y[rank(x)<a]) & trading_data$Trdmnt== trading_months[i]]
  seashort$mvol <- trading_data$Msmvosd[is.element(trading_data$Stkcd,y[rank(x)<a]) & trading_data$Trdmnt== trading_months[i]]
  ...
  sealong$vol <- trading_data$Msmvttl[is.element(trading_data$Stkcd,y[rank(x)>b]) & trading_data$Trdmnt== trading_months[i]]
  sealong$mvol <- trading_data$Msmvosd[is.element(trading_data$Stkcd,y[rank(x)>b]) & trading_data$Trdmnt== trading_months[i]]
  ...
  #按指标进行5等分，看平均收益
  seaport$r_group1[i] <- mean(trading_data$Mretwd3m[is.element(trading_data$Stkcd,y[rank(x)>(stockn *0.8)]) & trading_data$Trdmnt== trading_months[i]],na.rm=TRUE)
  seaport$r_group2[i] <- mean(trading_data$Mretwd3m[is.element(trading_data$Stkcd,y[(rank(x)<(stockn *0.8)) & (rank(x)>(stockn *0.6))]) & trading_data$Trdmnt== trading_months[i]],na.rm=TRUE)
  seaport$r_group3[i] <- mean(trading_data$Mretwd3m[is.element(trading_data$Stkcd,y[(rank(x)<(stockn *0.6)) & (rank(x)>(stockn *0.4))]) & trading_data$Trdmnt== trading_months[i]],na.rm=TRUE)
  seaport$r_group4[i] <- mean(trading_data$Mretwd3m[is.element(trading_data$Stkcd,y[(rank(x)<(stockn *0.4)) & (rank(x)>(stockn *0.2))]) & trading_data$Trdmnt== trading_months[i]],na.rm=TRUE)
  seaport$r_group5[i] <- mean(trading_data$Mretwd3m[is.element(trading_data$Stkcd,y[rank(x)<(stockn *0.2)]) & trading_data$Trdmnt== trading_months[i]],na.rm=TRUE)
...
write.csv(seaport,file ="results\\value\\PCF-portfolio.csv",)
write.csv(results_des,file ="results\\value\\PCF-result.csv",)
