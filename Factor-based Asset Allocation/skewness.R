...
skewness_factor<-data.frame(Stkcd = unique(trading_data$Stkcd),skewness=matrix(NA,data_length,1))
...
for(i in unique(trading_data$Stkcd)){
  ...
  skewness_factor[which(skewness_factor$Stkcd==i),2]<-skewness(data$Mretwd12m)
...
for (i in 1:nmonth){
  ...
  x <- skewness_factor$skewness[trading_data$Trdmnt == trading_lastmonth]
...
write.csv(seaport,file ="results\\liangjia\\skewness-portfolio.csv",)
write.csv(results_des,file ="results\\liangjia\\skewness-result.csv",)
