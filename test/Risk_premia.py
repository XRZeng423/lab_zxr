...
def PCAFM(rt, gt, pmax, q):
    ...
    for phat in range(pmax):  # % at least one factor up to pmax
        ...
        vhat    = randn(phat+1, T)
        betahat = randn(n, phat+1)
        Gammatilde = np.zeros([phat + 1, 1])
        etahat = np.zeros([d, phat + 1])
        what = np.zeros([d, T])
    ...
    #Gammatilde = inv(np.column_stack((np.ones(n),betahat)).T @ np.column_stack((np.ones(n),betahat))) @ np.column_stack((np.ones(n),betahat)).T @ np.mean(rt, axis=1).reshape(-1, 1)
    ...
    ##############################################################################
        ...
        Gammahat[:, phat] = etahat @ Gammatilde[0:(len(Gammatilde))].flatten()
